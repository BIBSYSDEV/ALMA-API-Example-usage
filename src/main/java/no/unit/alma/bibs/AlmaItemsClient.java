...
public class AlmaItemsClient implements AlmaItems {
    ...
    public AlmaItemsClient(AlmaClient almaClient) {
    ...
    @Override
    public ItemLoan createUserLoanOnItem(final String barcode, final String user_Id, final String library, final String circulationDesk) {
    ...
    @Override
    public ItemLoan createUserLoanOnItem(final String barcode, final String user_Id, final String library, final String circulationDesk, final float fine, final LoanStatus loanStatus) {
        final Item item = getItem(barcode);
        final BibData bibData = item.getBibData();
        final HoldingData holdingData = item.getHoldingData();
        final ItemData itemData = item.getItemData();
    ...
    public UserRequests getRequestsFromItem(String mms_id, String holding_id, String item_pid, boolean deleted) {
        WebTarget requestsFromItemTarget = bibsTarget
            .path(mms_id)
            .path("holdings")
            .path(holding_id)
            .path("items")
        ...
        if (deleted) {
            requestsFromItemTarget = requestsFromItemTarget
    ...
    public UserRequest createPatronRequest(Item item, String pickupLocationLibrary, String message, String userId) {
        ...
        if (StringUtils.isEmpty(userId)) {
            throw new IllegalArgumentException("No ltId provided for barcode=" + item.getItemData().getBarcode() + ", could not create request");
        }
    ...
    @Override
    public UserRequest createDigitizationRequest(String barcode, UserRequest.TargetDestination targetDestination, String message) {
        final Item item = getItem(barcode);
        return createDigitizationRequest(item, targetDestination, message);
    }

    @Override
    public UserRequest createDigitizationRequest(Item item, UserRequest.TargetDestination targetDestination, String message) {
        return createDigitizationRequest(item, targetDestination, message, null);
    }

    @Override
    public UserRequest createDigitizationRequest(Item item, UserRequest.TargetDestination targetDestination, String message, String userId) {
        final BibData bibData = item.getBibData();
        final HoldingData holdingData = item.getHoldingData();
        final ItemData itemData = item.getItemData();
        if (bibData == null || holdingData == null || itemData == null) {
...
//            log.error("", e);
        ...
        WebTarget digitizationTarget = bibsTarget
            .path(bibData.getMmsId())
            .path("holdings")
            .path(holdingData.getHoldingId())
            .path("items")
        ...
        if (StringUtils.isNotEmpty(userId)) {
            digitizationTarget = digitizationTarget
                .queryParam("user_id", userId);
        }
    ...
    @Override
    public void deleteItem(final String mmsId, String holdingsId, final String itemId, boolean override, HoldingsRecord holdingsRecord) {
       bibsTarget
           .path(mmsId)
           .path("holdings")
           .path(holdingsId)
    ...
    public void deleteItem(final String barcode, boolean override, HoldingsRecord holdingsRecord) {
        ...
        final String holdingsId = item.getHoldingData().getHoldingId();
        final String itemId = item.getItemData().getPid();
        WebTarget deleteTarget = bibsTarget
            .path(recordId)
            .path("holdings")
            .path(holdingsId)
            .path("items")
    ...
    @Override
    public UserRequest updateCommentOnRequest(String barcode, String requestId, String comment, boolean appendToExistingComment) {
    ...
    @Override
    public UserRequest actionOnRequest(String barcode, String action, String requestId, boolean releaseItem) { //todo: releaseItem er ikke i bruk
        final Item item = getItem(barcode);
        final String recordId = "" + item.getBibData().getMmsId();
        final String holdingsId = item.getHoldingData().getHoldingId();
        final String itemId = item.getItemData().getPid();
